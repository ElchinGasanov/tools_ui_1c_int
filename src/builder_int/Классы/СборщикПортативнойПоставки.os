...
Процедура ВыполнитьКонвертацию(КаталогПортативнойСборки) Экспорт
	...
	СконвертироватьМодульГлавнойОбработки();
...
Процедура СкопироватьИсходныеФайлыДляКонвертации()
	...
	Для Каждого ТекКартинка Из ОписаниеРасширения.ОбщиеКартинки Цикл
		КаталогПоискаФайла=ОбъединитьПути(ОбъединитьПути(КаталогИсходныхФайловРасширения,"ToolsInternational","CommonPictures",ТекКартинка.Имя),"Ext","Picture");
...
Процедура ПреобразоватьКорневойФайлОбработкиДляПереводаЕеВоВнешнюю(ТекИнструмент)
	...
	Если СвойстваОбработки._Элементы["DefaultForm"].Свойство("_Значение") Тогда
		СвойстваОбработки._Элементы["DefaultForm"]._Значение = "External" + СвойстваОбработки._Элементы["DefaultForm"]._Значение;
	КонецЕсли;	
	...
	Если ТекИнструмент.ЭтоОтчет Тогда
		...
		Если Найти(СхемаПоУмолчанию, ИмяУзла)>0 Тогда
			СвойстваОбработки._Элементы["MainDataCompositionSchema"]._Значение = "External" + СхемаПоУмолчанию;
		КонецЕсли;
...
КонецПроцедуры
Процедура ВыполнитьДополнительныеЗамены(ИмяФайлаМодуля, ОписаниеИнструмента, ТипНастройкиЗамены)

	НастройкиЗамены = НастройкиЗамены(ОписаниеИнструмента);
	МассивЗамен = Новый Массив;

	Если НЕ НастройкиЗамены.Свойство(ТипНастройкиЗамены, МассивЗамен) Тогда
		Возврат;
	КонецЕсли;
	...
	ЕстьЗаменаОбщая = Ложь;

	Для Каждого ТекНастройка ИЗ МассивЗамен Цикл

		Если ТипЗнч(ТекНастройка.МестаПоиска) = Тип("Массив") Тогда
			Если ТекНастройка.МестаПоиска.Найти(ОписаниеИнструмента.Имя) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;

		ЕстьЗамена = (СтрНайти(Исходник, ТекНастройка.ПодстрокаПоиска) <> 0);

		Если ЕстьЗамена Тогда
			Исходник = СтрЗаменить(Исходник, ТекНастройка.ПодстрокаПоиска, ТекНастройка.ПодстрокаЗамены);	
		КонецЕсли;

		ЕстьЗаменаОбщая = ЕстьЗаменаОбщая ИЛИ ЕстьЗамена;

	КонецЦикла;

	Если ЕстьЗаменаОбщая Тогда
		ЗаписатьМодуль(Исходник, ИмяФайлаМодуля);
	КонецЕсли;

КонецПроцедуры

Функция НастройкиЗамены(ОписаниеИнструмента)

	НастройкиЗамены = Новый Структура;

	// ЭтоГлавнаяОбработка

	МассивЗамен = Новый Массив;
	// замены для открывателей общих форм

	МассивЗамен.Добавить(Новый Структура("МестаПоиска, ПодстрокаПоиска, ПодстрокаЗамены",
			Неопределено,
			"OpenForm(""CommonForm.UT_Settings"")",
			"OpenForm(""ExternalDataProcessor.UT_PortableUniversalTools.Form.UT_ToolsSettings"", , ThisForm)")
		);

	НастройкиЗамены.Вставить("ЭтоГлавнаяОбработка", МассивЗамен);

	// ЭтоМодульФормы

	МассивЗамен = Новый Массив;
	// замены для получателей локальных шаблонов

	МассивЗамен.Добавить(Новый Структура("МестаПоиска, ПодстрокаПоиска, ПодстрокаЗамены",
			Неопределено,
			"DataProcessors." + ОписаниеИнструмента.Имя + ".GetTemplate",
			"FormAttributeToValue(""Object"").GetTemplate")
		);

	МестаПоиска = Новый Массив;
	// замены для вызовов модулей менеджера

	МестаПоиска.Добавить("UT_DuplicateObjectDetection");
	МассивЗамен.Добавить(Новый Структура("МестаПоиска, ПодстрокаПоиска, ПодстрокаЗамены",
			МестаПоиска,
			"DataProcessors.UT_DuplicateObjectDetection.MetadataObjectSettings()",
			"UT_GENERATION_CommonModulesByName(""UT_DuplicateObjectDetection_ManagerModule"").MetadataObjectSettings()")
		);

	МестаПоиска = Новый Массив;
	// Замены для открывателей форм 
	// Пример до: OpenForm("DataProcessor.UT_JSONEditor.Form", Parameters);
	// Пример после: OpenForm("ExternalDataProcessor.UT_JSONEditor.Form", Parameters);

	// Common modules
	МестаПоиска.Добавить("UT_CommonClient");
	МестаПоиска.Добавить("UT_CodeEditorClient");
	// Tools
	МестаПоиска.Добавить("UT_ConfigurationNavigator");
	МестаПоиска.Добавить("UT_ConsoleWebServices");
	МестаПоиска.Добавить("UT_DCSEditor");
	МестаПоиска.Добавить("UT_LicenseInformation1C");
	МестаПоиска.Добавить("UT_ObjectReferencesSearch");
	МестаПоиска.Добавить("UT_ObjectsAttributesEditor");
	МестаПоиска.Добавить("UT_QueryConsole");
	МестаПоиска.Добавить("UT_ReportsConsole");
	МассивЗамен.Добавить(Новый Структура("МестаПоиска, ПодстрокаПоиска, ПодстрокаЗамены",
			МестаПоиска,
			"""DataProcessor.",
	...
	МестаПоиска = Новый Массив;
	// Замены для открывателей общих форм 
	// Пример до: OpenForm("CommonForm.UT_ConfigurationSourseFilesSaveSettings", FormParameters)
	// Пример после: OpenForm("ExternalDataProcessor.UT_PortableUniversalTools.Form.UT_ConfigurationSourseFilesSaveSettings", FormParameters)
	...
	МассивЗамен.Добавить(Новый Структура("МестаПоиска, ПодстрокаПоиска, ПодстрокаЗамены",
			МестаПоиска,
			"""CommonForm.",
			"""ExternalDataProcessor.UT_PortableUniversalTools.Form.")
		);

	НастройкиЗамены.Вставить("ЭтоМодульФормы", МассивЗамен);

	// ЭтоМодульОбъекта

	МассивЗамен = Новый Массив;
	// Замены для получателей общих шаблонов
	...
	НастройкиЗамены.Вставить("ЭтоМодульОбъекта", МассивЗамен);

	Возврат НастройкиЗамены;
...
Процедура СконвертироватьМодулиПортативнойПоставки()
	Для Каждого ТекИнструмент Из СтруктураПортативныхИнструментов.Инструменты Цикл
		...
		Для Каждого ИмяФормы Из ОписаниеИнструмента.Формы Цикл
			...
			ВыполнитьДополнительныеЗамены(ИмяФайлаФормы, ОписаниеИнструмента, "ЭтоМодульФормы");
		КонецЦикла;
	...
	Для Каждого ОбщийМодуль ИЗ СтруктураПортативныхИнструментов.ОбщиеМодули Цикл
		...
		ЭтоОбщегоНазначения=ОписаниеИнструмента.Имя="UT_CommonClientServer";
		...
		Если ОписаниеИнструмента.Клиент Тогда
			...
			ТекстОбщегоМодуля=ДобавитьДирективуМетодовМодуля(ТекстОбщегоМодуля,"&AtClient");
		...
		ВыполнитьДополнительныеЗамены(ИмяФайлаМодуля, ОписаниеИнструмента,"ЭтоМодульОбъекта");
		Если ЭтоОбщегоНазначения Тогда
...
Процедура ДополнитьГенерируемымКодомМодульФормыГлавнойОбработки()
	...
	ОбъединитьПути(СтруктураПортативныхИнструментов.ГлавнаяОбработка.Каталог,"Forms","Form"),
	"Ext",
	"Form",
	"Module.bsl");
...
Процедура ДобавитьВГлавнуюОбработкуМодульПрограммногоИнтерфейса(УзелОбработки)
	...
	ТекстовыйДокумент = Новый ТекстовыйДокумент();
	ТекстовыйДокумент.УстановитьТекст(ДобавитьДирективуМетодовМодуля(ИсходникМодуля, "&AtClient"));
...
Процедура СконвертироватьМодульГлавнойОбработки()

	Каталог = СтруктураПортативныхИнструментов.ГлавнаяОбработка.Каталог;
	ИмяФормы = "Form";
	ОписаниеИнструмента = Новый Структура;
	ОписаниеИнструмента.Вставить("Имя", СтруктураПортативныхИнструментов.ГлавнаяОбработка.Имя);

	ИмяФайлаФормы=ОбъединитьПути(ОбъединитьПути(Каталог,"Forms", ИмяФормы),
	"Ext",
	"Form",
	"Module.bsl");
	
	ВыполнитьДополнительныеЗамены(ИмяФайлаФормы, ОписаниеИнструмента, "ЭтоГлавнаяОбработка");

КонецПроцедуры

Процедура СобратьБинарныеФайлыПроекта(КаталогПортативнойСборки)
	ФС.ОбеспечитьКаталог(КаталогПортативнойСборки);
	КаталогОбработокСборки = ОбъединитьПути(КаталогПортативнойСборки, "Tools");
...
Лог = Новый Лог("app.build.tools_ui_1c_int");
