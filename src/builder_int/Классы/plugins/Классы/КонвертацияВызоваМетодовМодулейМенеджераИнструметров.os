...
Процедура Открыть(Парсер, Параметры) Экспорт
	...
	ТиповыеМетодыОбработок.Добавить("gettemplate");
	ТиповыеМетодыОбработок.Добавить("create");
...
Процедура ПосетитьВыражениеИдентификатор(Описание) Экспорт
	Если НРег(Описание.Голова.Имя)="dataprocessors" Тогда
		ЭтоОбработка=Истина;
	ИначеЕсли НРег(Описание.Голова.Имя)="reports" Тогда
		ЭтоОбработка=Ложь;
	Иначе
		Возврат;
	КонецЕсли;
	...
	Если ТиповыеМетодыОбработок.Найти(НРег(ЭлементМетода.Имя))=Неопределено Тогда
		Замена(ПрефиксПеременныхИПроцедур+"CommonModulesByName("""+ЭлементИмени.Имя+"_ManagerModule"")",  Описание.Начало, ЭлементИмени.Конец);
	ИначеЕсли НРег(ЭлементМетода.Имя)="create" Тогда
		Замена(ПрефиксПеременныхИПроцедур+"CommonModulesByName("""+ЭлементИмени.Имя+""")",  Описание.Начало, ЭлементМетода.Конец);
	Иначе
		Замена("ExternalDataProcessors.Create("""+ЭлементИмени.Имя+""")", Описание.Начало, ЭлементИмени.Конец);
	КонецЕсли;
КонецПроцедуры
