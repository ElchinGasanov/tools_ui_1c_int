...
Процедура ПосетитьОбъявлениеМетода(Описание) Экспорт
	...
	Если Описание.Операторы.Количество()>0 Тогда
		...
		Если Описание.Сигнатура.Тип=Типы.ОбъявлениеСигнатурыФункции Тогда
			ТекстПроцедуры="Return "+ТекстПроцедуры;
		КонецЕсли;
...
Процедура ПокинутьМодуль(Описание) Экспорт
	...
	|&AtServer
	|Function " + ПрефиксПеременныхИПроцедур + "ModuleObject()
	|	Return FormAttributeToValue(""Объект""); // Object - not work.
	|EndFunction
	|
	|";
	Для Каждого Метод Из МассивМетодов Цикл
		...
		Если Метод.Тип=Типы.ОбъявлениеСигнатурыФункции Тогда
			ТекстСерверныхВызовов=ТекстСерверныхВызовов+"Return ";
		КонецЕсли;

		ТекстСерверныхВызовов=ТекстСерверныхВызовов+ПрефиксПеременныхИПроцедур+"ModuleObject()."+Метод.Имя+"("+ТекстПараметров+");";
